...
\vspace{0.22cm}
\featurebox{Delay systems}{Fixed-delay DDEs retain only the required history in a memory-saving ring buffer.}
\end{column}
\begin{column}{0.49\linewidth}
...
No implicit stiff ODE solvers in the core package. Adaptive ODE integration (via Diffrax) is available for single ODEs only, not networks or DDEs, for tolerance control, not speed.}
