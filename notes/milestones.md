...
- **Definition of done - met:** `pytest tests/` passes 4/4:
  `test_x64_enabled`, `test_running_on_cpu`, and two plumbing checks - an
  uncoupled linear-decay trajectory matches `exp(gamma·t)` analytically
  ...
  the DDE-shaped plumbing before any Lyapunov math sits on top of it in
  M1/M4).
...
- **`src/lyapax/simulator/step.py`** (additive, backward-compatible -
  ...
  original `G_default`/`coup_a`/`coup_b`/`delay_steps`-based behavior is byte-
  for-byte unchanged when the new params are omitted; see
  ...
  custom `coupling_fn` aren't supported yet - `_read_delayed_coupling` bakes
  the coupling formula into the per-edge gather inline (different edges read
  different time offsets, so there's no single per-node "delayed cvar_state"
  to hand a plain `coupling_fn`); that needs an edge-aware `coupling_fn`
  signature, a real design fork left for M5.
...
- The two Tier-4 benchmarks are expressed as 1-node self-loop
  ...
  (mirroring `test_network.py`'s locally-defined `ModelSpec` helpers), not in
  `src/lyapax/systems.py`, since they now need the ModelSpec/coupling
...
- **`vmap` over parameter grids / initial conditions for LE-vs-parameter
  ...
  `make_parametrized_network_step_fn` takes `params` as a call-time argument
  instead (`(state, params) -> new_state`), and `params` alone is swept with
  `jax.vmap`.
...
- **GPU smoke test - ✅ done (this session).** M0's blocker
  ...
  `cuda:0`, not just device-enumerates), then via the new `tests/test_gpu.py`.
  **Skipped by default**, opt in with `JAX_PLATFORMS=cuda pytest
  tests/test_gpu.py`: `tests/conftest.py` sets `JAX_PLATFORMS=cpu` via
  `os.environ.setdefault`, which only applies if the variable isn't already
  set, so exporting it before pytest starts overrides the CPU default without
  touching `conftest.py` - the rest of the suite stays CPU-only and unaffected
  (`32 passed, 2 skipped` when run without the override, `2 passed` when run
  with it).
...
- [ ] Diffrax adapter for adaptive/stiff ODE integration - was tracked as
  ...
  see **M9** below.
- [ ] `pmap`/`shard_map` examples for multi-device parameter sweeps - M6's
      `sweep.py` is single-device `jax.vmap` only; document that explicitly
      and add a `jit(vmap(...))` example before considering multi-device.
...
- [x] **DDE resume (follow-up session, once a concrete need arose: the
  ...
  one-ring-cycle transient floor (M4's `min_transient = horizon * dt`) - a
  resumed run's checkpointed `buf` is already fully populated from real
  dynamics, so re-flooring would just waste steps, not fix a correctness gap.
  Validation mirrors the ODE resume tests one-for-one (`tests/test_dde.py`):
  `test_dde_resume_matches_single_uninterrupted_run` (two chunks via `resume`
  bit-for-bit match one uninterrupted call, same pattern as the ODE version),
  plus mutual-exclusivity-with-`t_transient`, mismatched-`k`, and dimension-
  mismatch rejection tests. Demo: `examples/17_dde_resume.py`, deliberately
  *not* reusing 16's linear-DDE system - a clean linear DDE converges in a
  single chunk (verified by hand before writing the demo, not illustrative of
...
- [ ] **Decision recorded, not deferred by omission:** DDE gets fixed-step
  ...
  item 1's resolution, ~order 4 until the interpolant itself caps it) - there
  is no known concrete need (stiff/multi-timescale delayed system) currently
  motivating the extra complexity.
...
- [x] No code changes to `lyapax/dde.py`. `dde_problem`/`network_dde_problem`
  ...
  *fixed-step* DDE-compatible builders, a different calling convention -
  `(state, dfun, coupling_at, dt, params)` - than `ode_problem`'s `(rhs, dt)
  -> step_fn`), so "no such parameter" wasn't a sufficient guard. Added an
  explicit check instead: `diffrax_adaptive_step`'s returned builder carries a
  duck-typed marker (`_lyapax_adaptive_ode_only = True`), and
  ...
  raises a clear `ValueError` naming the diffrax-DDE-support gap, instead of
  letting a mismatched-arity `TypeError` surface from deep inside the DDE
  stepping internals. Tested:
...
- [x] Differentiability: `test_jacfwd_matches_finite_difference` (works,
  ...
  originally-anticipated "non-smooth at accept/reject boundaries" caveat - see M9.2 item 3 above).
